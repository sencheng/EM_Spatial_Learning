...
class Learnable_DR():
    '''
    Default Representation (DR) metrics class. The DR matrix is either loaded or learned incrementally from the interactions with the env.
    
    | **Args**
    | numOfStates:                 num of states.
    '''
    
    def __init__(self, numOfStates, gamma):
        self.numOfStates = numOfStates
        self.gamma = gamma
        self.D = np.zeros((numOfStates, numOfStates))
        
    def updateD(self, state, nextstate, lr=0.1):
        '''
        Update the default representation with one single experience 
        '''
        # construct the TD error 
        I = np.zeros((self.numOfStates,))
        I[state] = 1
        TD_e = I + self.gamma*self.D[nextstate] - self.D[state]
        # update the DR matrix 
        self.D[state] += lr*TD_e
    ...
    def loadD(self, path):
        try:
	        ...
	        print('No pre-trained DR found, start learning DR...')
	        return False
        else:
            return True
