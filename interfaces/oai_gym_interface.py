...
class UnityInterface(gym.Env):
    ...
    class UnityProcessor(Processor):
        ...
        def __get_observation_space(self, env_agent_specs):
            ...
            processed_obs_shapes = []

            for observation_shape in observation_shapes:
                if len(observation_shape) == 3:
                    if self.gray_scale:
                        observation_shape = observation_shape[:2]
                processed_obs_shapes.append(observation_shape)

            return processed_obs_shapes
        ...
        def process_observation(self, observations):
            ...
            """
            observations = self.__process_observations(observations)

            ## Somehow this step of displaying processed obs is very important, since if omit this step the RAM
            ## usage of the laptop will be quickly filled up. The reason hasn't been figured out.
            for observation in observations:
                self.performance_monitor.display_processed_observation(observation)
            ###################################################################################################
            #print(observations.shape) (1, 84, 84, 3)
            return observations
        ...
        def __process_observations(self, observations):
            ...
            for i in range(len(observations)):
                if len(observations[i].shape) == 4: # if a double image observation is returned
                    observations[i] = observations[i][0]
                # apply when observation is an image
                if len(observations[i].shape) == 3:
                    if self.gray_scale:
                        observations[i] = self.__to_gray_scale_image(observations[i])
    ...
    def __init__(self, env_path, scene_name=None,
                 time_scale=3.0, nb_max_episode_steps=0, decision_interval=5,
                 agent_action_type='discrete', use_gray_scale_images=False,
                 modules=None,
                 seed=42, timeout_wait=60, side_channels=None,
                 performance_monitor=None, with_gui=False):
    ...
    def _step(self, action, *args, **kwargs):
        ...
        if self.nb_episode_steps >= self.nb_max_episode_steps-1:
            done = True
    ...
    def __start_editor_process(self, resource_path, scene_path, scene_name):
        ...
        self.editor_process = subprocess.Popen([os.environ['UNITY_EXECUTABLE_PATH'],
                                                '-createProject',
                                                '/home/xzeng/dev/unity_folder/projects/temp',
