...
class FrontendUnityOfflineInterface(FrontendUnityInterface):
    ...
    | **Args**
    | scenarioPath:                 The path of the scenario that should be processed.
    ...
    def __init__(self, scenarioPath=None):
        ...
        try:
            ...
            raise Exception('Offline environment for %s does not exist, please use script environments/generate_off_unity.py of cobel to generate it.' % scenarioPath)
        ...
        self.envData['topoData'] = None
        self.envData['imageIdx'] = None
    ...
    def step_simulation_without_physics(self, newNode, newYaw):
        ...
        self.envData['imageData'] = imageData
        self.envData['poseData'] = poseData
        self.envData['topoData'] = str([newNode, newYaw])
        self.envData['imageIdx'] = np.where(np.array(list(self.env.keys())) == str([newNode, newYaw]))[0][0]
        # return acquired data
        return imageData, poseData
    ...
    def numOfStates(self):
    	'''
    	It is able to extract the number of states from the env data.
    	'''
    	# get the number of states (num_node * num_orientation)
    	num_state = len(self.env.keys())-3
    	return num_state
