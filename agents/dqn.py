...
class DQNAgent(AbstractDQNAgent):
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            if type(state1_batch[0]) == type(None) or type(state0_batch[0]) == type(None) or type(reward_batch[0]) == type(None):
                # at the first step of every episode, the next state has not been observed so none,
