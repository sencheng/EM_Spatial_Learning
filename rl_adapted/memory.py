...
import random
import numpy as np
from rl_adapted.util import SumSegmentTree, MinSegmentTree
...
class NoMemory(Memory):
    ...
    def sample(self): # batch_size is not used here, only for compatible with the keras-rl package
...
class Separate_EM():
    # in this memory, each episode is stored separately
    def __init__(self):
        self.actions = []
        self.e_actions = []
        self.rewards = []
    ...
    def append(self, observation, action, reward, terminal, training=True):
        self.e_observations.append(observation)
        self.e_actions.append(action)
        self.e_rewards.append(reward)
        self.e_terminals.append(terminal)
        if terminal:
            self.observations.append(self.e_observations)
            self.actions.append(self.e_actions)
            self.rewards.append(self.e_rewards)
            self.terminals.append(self.e_terminals)
            self.e_actions = []
            self.e_rewards = []
            self.e_terminals = []
            self.e_observations = []
    ...
    def sample_episode(self):
        idx = np.random.randint(0, len(self.rewards))
        episode_len = len(self.observations[idx])
        experiences = []
        for i in range(episode_len):
            experiences.append(Experience(state0=self.observations[idx][i], action=self.actions[idx][i], reward=self.rewards[idx][i],
                                          state1=self.observations[idx][min(i+1, episode_len-1)], terminal1=self.terminals[idx][i]))

        return experiences

class NStepMemory(Memory):
    # e.g., we assume limit=10 here
    def __init__(self, limit, **kwargs):
        super(NStepMemory, self).__init__(**kwargs)
        self.limit = limit
    ...
    def append(self, observation, action, reward, terminal, training=True):
        super(NStepMemory, self).append(observation, action, reward, terminal, training=training)
        self.observations.append(observation)
        self.actions.append(action)
        self.rewards.append(reward)
    ...
    def sample(self):
        ...
        if len(self.rewards) < self.limit:
            return experiences
        # sample a random idx that we want to draw a sample
        idx = random.randint(0, self.limit-2) # avoid sampling the last experience
        # check if idx is behind a terminal idx or ahead of it
        idx_terminal=np.where(self.terminals)[0]
        if len(idx_terminal)!=0:
            idx_terminal=idx_terminal[0]
            if idx <= idx_terminal:
                end_idx = idx_terminal
            else: end_idx = self.limit-1
        else:
            end_idx = self.limit-1
        # return a sequence of experiences start from idx and end with end_idx, where state1 is always the end state

        for i in range(idx, end_idx+1):
            state0=[self.observations[i]]
            state1=[self.observations[end_idx]]
            reward=self.rewards[i]
            action=self.actions[i]
            terminal=self.terminals[end_idx]
            experiences.append(Experience(state0=state0, action=action, reward=reward,
                               state1=state1, terminal1=terminal))
...
class SequentialMemory(Memory):
    ...
    def sample(self, batch_size, batch_idxs=None):
        ...
        assert self.nb_entries >= self.window_length, 'not enough entries in the memory'
