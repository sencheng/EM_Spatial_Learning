...
from rl_adapted.callbacks import (
    CallbackList,
    TestLogger,
    TrainEpisodeLogger,
    TrainIntervalLogger,
...
class Agent(object):
    ...
    def fit(self, env, nb_episodes, action_repetition=1, callbacks=None, verbose=1,
        ...
        observation = None
        episode_reward = None
        step_rewards = []   # a list which contains the reward for each step within an episode. Used for logging
        episode_step = None
        episode_actions = [] # a list to store all the actions that the agent take within 1 episode
        did_abort = False
        episode_metrics = []
        try:
            while self.episode < nb_episodes:
                ...
                # remove the nan metrics during the warm up phase
                if not np.isnan(np.sum(metrics)):
                    episode_metrics.append(metrics)
                episode_reward += reward
                # collect the reward of each step and log it at the end of an episode
                ...
                if done:
                    ...
                    episode_logs = {
                        ...
                        'metrics': np.mean(episode_metrics, axis=0)
                    }
    ...
    def test(self, env, nb_episodes=1, action_repetition=1, callbacks=None, visualize=True,
		...
		# Here in the test trial, we still keep the randomness, so set self.training = True
        self.training = True
        self.step = 0
        ...
        for episode in range(nb_episodes):
            ...
            while not done:
                ...
                # self.backward(reward, terminal=done) # why backward???
                episode_reward += reward
