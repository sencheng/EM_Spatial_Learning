...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
        ...
        elif isinstance(self.memory, NoMemory):
            self.memory_type = 'online'
    ...
    def replay(self, metrics):
        ...
        if self.memory_type=='online':
            if type(state1_batch[0])==type(None) or type(state0_batch[0])==type(None): # at the first step of every episode, the next state has not been observed so none,
                return metrics               # then we skip the backward step
